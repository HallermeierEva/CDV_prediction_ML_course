...
class ImprovedModel:
    """
    XGBoost model for improved CVD prediction.
    """
    ...
    def train(self, X_train, y_train, use_random_search=True):
        """
        ...
        """
        config.print_subsection_header("Training XGBoost Model")
        ...
        if use_random_search:
            ...
            print(f"  - Iterations: {config.N_ITER_RANDOM_SEARCH}")
            ...
            random_search = RandomizedSearchCV(
                base_model,
                config.XGB_PARAM_DISTRIBUTIONS,
        ...
        if hasattr(X_train, 'columns'):
            # Get importance scores (gain, weight, cover)
    ...
    def get_feature_importance(self, importance_type='gain', top_n=10):
        ...
        return self.feature_importance.nlargest(top_n, importance_type)
    ...
    def save_model(self, filepath=None):
        ...
        filepath : str or Path, optional
            Path to save the model. If None, uses config.XGB_MODEL_FILE
        """
        if filepath is None:
            filepath = config.XGB_MODEL_FILE
    ...
    def save_results(self, filepath=None):
        ...
        -----------
        filepath : str or Path, optional
            Path to save results. If None, uses config.XGB_RESULTS_FILE
        """
        if filepath is None:
            filepath = config.XGB_RESULTS_FILE
        ...
        if self.feature_importance is not None:
            results_to_save['feature_importance'] = {
                ...
                'gain': self.feature_importance['gain'].tolist(),
                'weight': self.feature_importance['weight'].tolist(),
                'cover': self.feature_importance['cover'].tolist()
            }
    ...
    def load_model(self, filepath=None):
        ...
        -----------
        filepath : str or Path, optional
            Path to the model file. If None, uses config.XGB_MODEL_FILE
        """
        if filepath is None:
            filepath = config.XGB_MODEL_FILE
...
def train_and_evaluate_xgboost(X_train, X_test, y_train, y_test):
    """
    Convenience function to train and evaluate the XGBoost model.
    ...
    """
    config.print_section_header("IMPROVED MODEL: XGBOOST")
    
    # Initialize model
    model = ImprovedModel()
    
    # Train
    model.train(X_train, y_train, use_random_search=True)
    ...
    print(model.get_feature_importance(importance_type='gain', top_n=10).to_string(index=False))
    
    # Save model and results
    model.save_model()
    model.save_results()
    
    config.print_section_header("XGBOOST MODEL COMPLETE")
